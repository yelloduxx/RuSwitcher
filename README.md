...
Use `--input fixtures.jsonl` for custom cases and `--learn-output rules.json` to produce supervised adaptive rules for missed expected conversions. The separate sequential `scripts/run_hid_integration_tests.sh` suite posts `CGEvent` key events through the installed app and verifies the final editor text; it is reserved for event-tap, layout switching, punctuation, and duplicate-insertion regressions.
...
`--input fixtures.jsonl` принимает свои тесты, а `--learn-output rules.json` создаёт обученные правила для ожидаемых, но пропущенных замен. Последовательный `scripts/run_hid_integration_tests.sh` оставлен только для реального event tap: смены раскладки, пунктуации и защиты от дублей.

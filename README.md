...
Typed `ghbdtn` when you meant `привет`? By default, tap **Shift twice** and RuSwitcher converts the last word in place, without copy-paste. Manual conversion works with any two installed layouts; automatic conversion is tuned and tested primarily for EN↔RU. The trigger remains configurable, and auto-conversion can fix a wrong layout at the end of a word.
...
This fork is installed separately as **RuSwitcher Lab**, so it can coexist with
...
ditto "RuSwitcher Lab.app" "/Applications/RuSwitcher Lab.app"
```

Requires macOS 13+, Xcode Command Line Tools and a code-signing identity. Do
not replace the author's `/Applications/RuSwitcher.app` when comparing builds.
...
Набрали `ghbdtn` вместо `привет`? По умолчанию нажмите **Shift два раза** — и RuSwitcher сконвертирует последнее слово прямо у каретки, без копипасты. Ручная конвертация работает с любой парой установленных раскладок; автоконверсия прежде всего настроена и проверена для EN↔RU. Триггер настраивается, а автоконверсия исправляет неверную раскладку на границе слова.
...
- **Обычный путь без буфера обмена** — автоконверсия и обычная ручная замена печатают текст напрямую. Редкий fallback для выделения сохраняет типы pasteboard и восстанавливает прежнее содержимое, только пока оно принадлежит RuSwitcher.
...
Форк устанавливается отдельно как **RuSwitcher Lab**, поэтому оригинальный
...
Требуются macOS 13+, Xcode Command Line Tools и identity для подписи кода. Не
заменяйте `/Applications/RuSwitcher.app` автора, если сравниваете сборки.
